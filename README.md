...
呼び出し側ではこのワークフローをコミット SHA に固定し、`pull_request_target` と PR 用 CI の `workflow_run` から呼び出します。書き込み権限を持つ処理では PR のコードを checkout・実行せず、PR 作成者、リポジトリ、head SHA、Dependabot メタデータと CI 結果を検証します。具体的な呼び出し設定、CI 名、マージ後のデプロイ起動は各リポジトリで管理します。
