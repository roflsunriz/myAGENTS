...
2. PR のコードを実行するジョブに書き込み権限を与えず、PR 作成者・head リポジトリ・head SHA・更新種別・CI 結果の照合を維持する。分類が CI 完了より遅い場合の再確認用 `callback_workflow_file` を呼び出し側の実ファイル名に合わせる。決定的な自動修復を有効にする場合は、コミット可能な生成物パスを完全一致で限定し、修復後の再検証 workflow に秘密情報や書き込み権限が露出しないことを監査する。`GITHUB_TOKEN` が起動した `workflow_dispatch` の後続 `workflow_run` に依存せず、返された run ID を直接待つ。CI 名は各利用リポジトリの実際の workflow 名に合わせる。
